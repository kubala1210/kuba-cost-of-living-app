class City:
    ...
    def cost_counter(self):
        ...
        print(f'Całkowity koszt życia w {self.country}, {self.city_name} to {self.total_cost}')

class Household(City):

    def __init__(self, country='', city_name='', cost_of_living=0,
                 cost_of_rent=0, total_cost=0, minimum_salary=0, average_salary=0, family_type='', r=1, multiplier=1):
    ...
    def minimum_salary_counter(self):
        ...
        return f'Minimalne wynagrodzenie w {self.country}, {self.city_name} to: {self.minimum_salary}'
    ...
    def wealth_level_counter(self, wealth_level=0, result=''):
        ...
        wealth_level = round(((self.average_salary * self.multiplier) / self.minimum_salary), 1)
        if wealth_level <= 1:
            result = f'Poziom zamożności wynosi {wealth_level}. {self.country}, {self.city_name} jest mało zamożne'
        if wealth_level > 1 and wealth_level <= 1.5:
            result =  f'Poziom zamożności wynosi {wealth_level}. {self.country}, {self.city_name} jest średnio zamożne'
        if wealth_level > 1.5 and wealth_level <= 2:
            result =  f'Poziom zamożności wynosi {wealth_level}. {self.country}, {self.city_name} jest zamożne'
        if wealth_level > 2:
            result = f'Poziom zamożności wynosi {wealth_level}. {self.country}, {self.city_name} jest bardzo zamożne'
        print(result)
...
class Single(Household):
    ...
    def minimum_salary_counter(self):
        ...
        print(f'Minimalne wynagrodzenie to: {self.minimum_salary}')
